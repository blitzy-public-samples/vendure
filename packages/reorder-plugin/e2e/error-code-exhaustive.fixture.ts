...
export function describeShopErrorCode(code: ErrorCode): string {
    ...
    // THE ASSERTION, EXHAUSTIVE HALF. `code` narrows here to the four members the live schema added rather
    // than to `never`, so this assignment is refused with TS2322 and the diagnostic names them. Do not add a
    // `default` clause, a type assertion or a suppression comment to silence it: any of those would
    // relocate the failure and destroy the evidence. This branch, and nothing above it, is what differs
    // from `error-code-defaulted.fixture.ts` — see notes 1 and 3 of the shared header.
