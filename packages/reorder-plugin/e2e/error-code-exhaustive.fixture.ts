...
export function describeShopErrorCode(code: ShopErrorCodeWithReorder): string {
    ...
    // THE ASSERTION, EXHAUSTIVE HALF. `code` is `ReorderErrorCode` here rather than `never`, so this
    // assignment is refused with TS2322 and the diagnostic names the four unhandled members. Do not add a
