...
describe('the single-list read reconciles the stored lineCount before the parent is exposed', () => {
    ...
    it('retries a pre-resolve read that failed once, and still returns a reconciled parent', async () => {
        ...
        // THIS CASE READS AS IT DOES. An earlier revision forgave a failed pre-resolve and let the parent
        // through unreconciled, on the argument that a read which would otherwise have succeeded should not
        // fail for the sake of a counter it only meant to tidy. That produces a WRONG ANSWER rather than an
        // error: the nested field resolver then reads its own page, and if THAT read succeeds it repairs the
        // row — after the executor has already taken `lineCount` off this object. The row ends up right and
        // the answer the buyer received stays wrong, which is precisely the ordering
        ...
        // Two attempts, one repair, and the CORRECTED value already on the object being returned.
        expect(returned).toBe(row);
        expect(service.getLinesForLists).toHaveBeenCalledTimes(2);
        expect(service.reconcileLineCount).toHaveBeenCalledTimes(1);
        expect(returned.lineCount).toBe(1);

        // And the retried page was cached, so the nested field is still served without a third read.
        const page = await entity.lines(ctx, returned, linesArgs());
        expect(page.totalItems).toBe(1);
        expect(service.getLinesForLists).toHaveBeenCalledTimes(2);
    });
    ...
    it('propagates a pre-resolve failure that survives the retry rather than answering stale', async () => {
        ...
        await expect(shop.activeCustomerReorderList(ctxFor(), { id: LIST_ID }, info)).rejects.toThrowError(
            'the nested read keeps failing',
        );

        // Bounded rather than open-ended: exactly two attempts, and no repair issued from a page never read.
        expect(service.getLinesForLists).toHaveBeenCalledTimes(2);
        expect(service.reconcileLineCount).not.toHaveBeenCalled();
    });

    it('leaves a malformed nested window to the field it belongs to, unretried', async () => {
        // THE ONE FAILURE CLASS THAT IS NOT RETRIED AND NOT RAISED FROM THE ROOT. A `UserInputError` is
        // attributable to the request's own arguments — an over-limit nested `take`, refused by the platform's
        // own `parseTakeSkipParams` — so retrying only repeats it, and raising it from the parent would report
        // the nested collection's bound at the ROOT's path when the error's path is the only proof the bound is
        // enforced on the nested collection at all. It cannot produce the hazard the retry exists for either: a
        // deterministic refusal of the same arguments fails in the nested resolver too, so no response goes out
        // carrying a reconciled row behind a stale scalar.
        const ctx = ctxFor();
        const info = infoFor(
            `query Q($id: ID!) { activeCustomerReorderList(id: $id) { lineCount lines(options: { take: 500 }) { totalItems } } }`,
        );
        service.getLinesForLists.mockRejectedValue(new UserInputError('error.take-limit-exceeded' as never));

        const returned = present(await shop.activeCustomerReorderList(ctx, { id: LIST_ID }, info));

        // One attempt, no repair, the stored counter reported as it stands — and the refusal still waiting for
        // the nested field, which meets it on its own path.
        expect(service.getLinesForLists).toHaveBeenCalledTimes(1);
        expect(service.reconcileLineCount).not.toHaveBeenCalled();
        expect(returned.lineCount).toBe(3);
