...
Twenty entries in two groups — thirteen product and seven architectural. **Fifteen of the twenty are open. Five are recorded as closed in place rather than deleted — product entries 2, 6 and 13, and architectural entries 1 and 7**, each because a closed decision and its reasoning are worth more to a reader than a gap, and because every reference to a numbered entry elsewhere in this set — architectural decision 5 in particular, which four files cite as a sign-off gate — continues to resolve when the numbering does not move. **Product entry 13 is the most recent of the five and it closed for the strongest of the reasons any of them closed:** it offered a maintainer a route-parameter design that the owning feature's own definition of done refuses, so the choice it presented could not have been built either way.
...
13. **CLOSED — no administrative route in this epic carries a customer-linked identifier, and this entry is retained in place rather than removed. Blocks 0 stories.** The entry keeps its position and its ordinal so that every existing reference to a numbered product entry still resolves. *A decision with a product and a privacy component; the codebase forced the question and then answered it.* **What closed it is a property of the identifiers rather than a preference between two workflows:** a `Customer` identifier and an attempt identifier are sequential auto-increment values by default [packages/core/src/config/default-config.ts:L101], so a route carrying one is both enumerable and a durable pointer to one named person in three stores the page does not control — browser history, the `Referer` header of any request the page originates to another origin, and any deployment-level telemetry that records URLs — **and all three outlive the moment the subject is cleared**. The owning feature therefore requires the route to carry **no** subject identifier and no attempt identifier, with both held in page state and sent as GraphQL variables inside the `POST` body, and it withdraws deep-linking as a page property with the cost named rather than hidden: a reload returns the page to its no-subject state, and a colleague is handed a buyer reference out of band instead of a link [tickets/EPIC-001/FEATURE-001-08-recurring-demand-visibility.md:§5. Definition of Done (Feature-Level)]. Story 08-03 owns the three assertions that prove the absence — the address bar carries no identifier, the history stack gains no entry naming one, and no request the page originates carries one in a `Referer` [tickets/EPIC-001/FEATURE-001-08/STORY-001-08-03-support-agent-reorder-lookup.md:§4.3.3 The Subject Binding Is A Safety Contract, Not A Convenience]. **Why this had to close rather than stay open with two costed answers, which is what an earlier version of this entry did:** that version presented *accept the route identifier, with four controls narrowing it* against *require a short-lived non-linkable handle*, said the story was buildable against either because "the addressing surface is the route parameter in both cases", and left the ruling gating sign-off alone. Every part of that is now wrong. The route parameter is the addressing surface in **neither** case, so the two answers were not two configurations of one design; the four controls it listed — an opaque route value, a history entry replaced rather than appended, the identifier removed from the route when the subject is cleared, and no cross-origin request — are controls over a route value this surface does not have, and three of the four are unstateable without one; and an entry offering a maintainer a design the owning feature's definition of done already refuses would have invited a choice that cannot be built. **What a deployment may still decide is nothing here**, because the residual privacy exposure the open version was weighing — administrative-lookup identifiers in an operator's URL telemetry — is not reachable once no identifier is in a URL.
...
# Longest first, so "twenty-one" is never read as "twenty". Used by the section-10-against-section-7
# reconciliation below, which reads a stated scenario count out of the story's own words.
...
for path in sorted(glob.glob('tickets/**/STORY-*.md', recursive=True)):
    ...
    # SECTION 10 AGAINST SECTION 7 -- a definition-of-done item that states its own scenario count in
    # words must state the count section 7 actually carries. This check exists because a story carried
    # five scenarios and a definition-of-done item requiring "every one of the four scenarios in
    # section 7", which put the fifth inside the story and outside its own gate: a suite omitting it
    # satisfied the item and reported success. Every step above counted the scenarios and none read the
    # sentence that gates them. The number is parsed from the story's own words, so the file stays the
    # authority and this check only requires it to agree with itself.
    for number, text in body(10):
        for said in re.finditer(r'\b(' + '|'.join(NUMBER_WORDS) + r')\s+(?:of\s+the\s+)?'
                                r'scenarios?\b', text, re.IGNORECASE):
            require(NUMBER_WORDS[said.group(1).lower()] == len(scenarios),
                    f'{path}:{number}: a definition-of-done item requires '
                    f'{said.group(1).lower()} scenario(s), section 7 carries {len(scenarios)}')
...
WITHDRAWN = [
    ...
    # THE EMPHASIS AND THE INTERVENING NOUN ARE WHY THIS RULE MISSED THREE LIVE SITES. Its first
    # form required "four" to be followed by whitespace and then optionally "administrative " and then
    # "definitions", which cannot match the way this corpus actually writes the claim -- bold and
    # italic markers around the number and the adjective, and the word "permission" between the
    # adjective and the noun: "the **four** *administrative* permission definitions ... publish
    # **five** members". Three passages carried exactly that and this step reported success on all
    # three. The tolerant separator below is bounded to emphasis, spacing and one or two intervening
    # words so it cannot drift into matching unrelated numbers, and the same tolerance is applied on
    # the members side. The bare "five published members" alternative is retained.
    ('permission ledger: withdrawn arithmetic',
     re.compile(r'four[*_\s]*(?:[A-Za-z`]+[*_\s]+){0,3}definitions?[^.]{0,120}?'
                r'five[*_\s]*(?:[A-Za-z`]+[*_\s]+){0,3}members'
                r'|five\s+published\s+members'
                r'|four\s+registrations\s+in\s+the\s+epic', re.IGNORECASE),
     'the ledger is three definitions yielding four published Permission members'),
    # THE PROSE-LEVEL HALF OF A SINGLE-AUTHORITY RULE, and the reason it is a WITHDRAWN-form rule
    # rather than a second SINGLE_AUTHORITY entry: the entry below already proves the SDL declaration
    # of this type occurs exactly once and in FEATURE-001-02, and it passed while two PROSE passages
    # assigned the declaration to FEATURE-001-06 -- one of them contradicted by its own file twelve
    # lines later. A schema-shape check cannot see a sentence, so the sentence needs its own rule.
    # Scoped to this type: the error name or the phrase naming it must sit in the same clause as the
    # attribution, so an unrelated statement about what FEATURE-001-06 does declare is not flagged.
    ('permission error declaration: withdrawn authority',
     re.compile(r'(?:`?ReorderPermissionDeniedError`?|permission refusal)[^.;]{0,60}?'
                r'declared\s+by\s+[*_]{0,2}FEATURE-001-06'
                r'|(?:`?ReorderPermissionDeniedError`?|permission refusal)[^.;]{0,60}?'
                r'FEATURE-001-06\s+declares', re.IGNORECASE),
     'FEATURE-001-02 is the single SDL authority for ReorderPermissionDeniedError because its own '
     'frozen union names it from that union\'s first version; FEATURE-001-06 only RETURNS it'),
    # The lookahead is load-bearing: without it this pattern also matches the first four words of
    # "one hundred and two hundred", which is the acceptance-criterion range V1 asserts, and reports
    # a stage-total defect on a sentence about criterion counts.
    ('permission ledger: withdrawn stage total',
     re.compile(r'one hundred and two(?!\s+hundred)', re.IGNORECASE),
...
for path in paths:
    ...
    for index, line in enumerate(lines, 1):
        if any(first <= index <= last for first, last in spans):
            continue
        # THE EXEMPTION IS SENTENCE-SCOPED, NOT LINE-SCOPED, AND THAT IS A CORRECTION RATHER THAN A
        # REFINEMENT. This sub-section's own invariant list forbids a line-wide exemption in terms --
        # "never to the whole line" -- because a line-wide pardon silently forgives every other
        # occurrence on that line. This step nevertheless implemented one, and in a corpus whose
        # paragraphs are single very long lines that is a paragraph-wide pardon: a passage assigning
        # this set's permission error result to the wrong declaring feature sat on the same line as an
        # unrelated withdrawal note and was forgiven by it, which is how it survived every gate. The
        # match is therefore tested against the sentence it falls in, and every match is reported
        # rather than the first, so two defects on one line are two findings.
        bounds = [0] + [m.end() for m in re.finditer(r'(?<=[.!?])\s', line)] + [len(line)]

        def exempt_at(position):
            for start, end in zip(bounds, bounds[1:]):
                if start <= position < end:
                    return bool(WITHDRAWAL.search(line[start:end]))
            return bool(WITHDRAWAL.search(line))

        for name, pattern, remedy in WITHDRAWN:
            for match in pattern.finditer(line):
                if exempt_at(match.start()):
                    continue
