...
The **requirements are proposals**: argue, rewrite or delete them. The **findings are reported facts**, read out of the platform at one recorded point and evidenced below. Six questions block all but one, so this is scoping, not specification.
...
- Every technical identifier and all evidence sit in the appendices, so this body can be rewritten without touching them.
- A marker closes the body; each question ends with a blank answer line.

## 4. The Problem

A buyer returning for the same things rebuilds the order by hand every time. Prices move and items run out between purchases, so a buyer told nothing finds out at checkout.
...
Six things already ship; proposing them as new work would be wrong. Adding many items at once reports an outcome per item, so partial success is established. Availability is three plain states, in stock, low stock or out of stock, never a quantity. Order history is buyer-tied, storefront-confined and ordered by when each order was placed, its items and quantities readable. Buyer preferences need no new store of data. The cart compares prices, with one limit. Reporting reaches a daily order count, its total and an average order value.

That limit: the comparison covers only movement since a line went into the cart, guarded by its own automated suite, so it says nothing about the price last paid. Nothing implements repeat ordering, replenishment, repeat-demand reporting or reminders.

## 7. What a Buyer Will Be Able To Do

**J1 — The weekly restock.** A buyer repeats a past order in one act: every item in the cart at last time's quantity, their own storefront only, regular items the same way. [REQ-001, REQ-004, REQ-007, REQ-022]
...
REQ-011 and REQ-017 appear in no journey: the first deferred pending an open question, the second a delivery obligation with no buyer-visible step.

## 8. Proposed Requirements

Seven capability areas, named the same way throughout: **CA-1 Reorder from a past order** (REQ-001 to REQ-004); **CA-2 Regularly purchased items** (REQ-005 to REQ-008); **CA-3 Saved reorder lists** (REQ-009 to REQ-011); **CA-4 Change awareness before commit** (REQ-012 to REQ-015); **CA-5 Reorder reminders** (REQ-016 to REQ-018); **CA-6 Recurring demand visibility** (REQ-019 to REQ-021); **CA-7 Storefront integration surface** (REQ-022).

Origins are four, one to a row: a clause of the Objective Statement quoted verbatim, or an inference from the Business Targets, the Primary Users, or a codebase finding.

Recomputed from the table below: 22 proposals — 7 quoting a clause, 4 from the Business Targets, 5 from the Primary Users, 6 from a codebase finding.
...
|REQ-004|Confine a repeat to one storefront|Returning Buyer, Storefront Developer|Inferred from a codebase finding|REQ-001|
...
|REQ-015|Let an agent see what was shown and accepted|Customer Support Agent|Inferred from the Primary Users|REQ-012, REQ-014|
|REQ-016|Reminders by opt-in only|Returning Buyer|Inferred from the Business Targets|REQ-005|
|REQ-017|Reminder delivery behind one interface, defaulting to no outside service and an in-platform record|Storefront Developer|Inferred from a codebase finding|REQ-016|
|REQ-018|Stop reminders wholesale or per item, honoured immediately|Returning Buyer|Inferred from the Primary Users|REQ-016|
|REQ-019|Show most repurchased items in the viewer's range|Marketplace Category Manager|Inferred from the Business Targets|REQ-006|
|REQ-020|Prepare repurchase figures on a schedule|Marketplace Category Manager, Seller Operations Manager|Inferred from a codebase finding|REQ-019|
|REQ-021|Confine a seller's figures to their own goods|Seller Operations Manager|Inferred from the Business Targets|REQ-019|
...
Contained but for one exception: attach this capability's information to what the platform already exposes, and something existing storefront calls receive changes shape, a violation to report. Avoidable, and it bears on CA-7 Storefront integration surface.

### What changes for people who never use this feature

Six behaviours were checked; five are untouched. Existing storefront calls keep their shape and result on the additive decision above; order history, the cart, the catalogue and availability are only read, so CA-1 Reorder from a past order and CA-4 Change awareness before commit disturb nothing; reporting is unchanged. The sixth, the permission values every administrative client receives, grows only if this work defines its own; both shipped reporting extensions reuse an existing one, so the choice is a longer list for everyone against closer control of who sees seller figures.

### What this work would collide with

Two. There is no built-in marketplace: one is assembled a storefront per seller by an addition, so seller-scoped reporting inherits that shape. And the vendor's plans put buying organisations, quotations and approvals in a paid layer above the open platform, so anything built now for shared buying either duplicates that layer or is replaced by it — which defers CA-3 Saved reorder lists and makes the deferral a purchase decision, not a wait.

### Where the risk concentrates

In two places. First, CA-2 Regularly purchased items: past items are readable, but no request answers which items recur, so anything asking must read a buyer's history through, a wholly new computation that CA-6 Recurring demand visibility waits on. Second, scope enforcement: every reporting proposal is a place one seller could see another's figures, every stored preference a place one buyer could read another's.

### Verification burden created

Substantial. The suite guarding this behaviour runs against four separate data engines, and twenty-seven of the hundred specifications must pass on all four. Changing how data is stored also invalidates cached seed data.

### Reversibility

Partial. Whatever the addition stores persists until deliberately removed; nothing is cleaned up when a capability is withdrawn, bar orphaned preferences, already swept on a schedule. Switching off stops new writes but removes nothing, and a live instance must never run with schema synchronisation.
...
Everything is added, nothing existing altered; any change to what an existing call returns is a violation to report. The core is untouched and stored shapes only added to, so this arrives as a self-contained addition. Prices are whole numbers in the smallest unit of currency, so a difference is the price last paid against the price now. Every request belongs to one storefront and one language, and nothing crosses it. No outside hosted service is required: anything wanting one sits behind an interface defaulting to the platform's own storage. And no measure is invented; absence is reported instead.
...
**Q5 — Are reminders strictly opt-in, and how should they reach the buyer?** Two choices. Whether: opt-in, or default-on with an easy way out; opt-in contacts nobody unasked but takes up slowly, default-on reaches more but contacts some who did not ask, costing trust. How: existing message sending, or a new interface keeping its own record; existing sending reaches buyers today but records nothing, so nobody can confirm what was sent. Recommendation: opt-in, behind the interface. Blocks REQ-016, REQ-017, REQ-018.

Answer:

**Q6 — Which administrative surface carries the seller-facing figures?** The newer, the older, or both. No buyer sees any: the newer follows the investment; the older still reaches staff there but its maintenance ends, so the work is done twice; both means two surfaces to keep aligned. Recommendation: the newer. Blocks REQ-019, REQ-021.

Answer:

## 14. Success Measures

Observable outcomes, no figures being available: a returning buyer repeats a past purchase without searching the catalogue; no price a buyer was shown changes without their saying so; an item that cannot be supplied is named before commitment; an operator sees what is bought again and again in their scope and no more; and a buyer who never asked for reminders never receives one.

Candidate metrics:

- Orders placed over time — **instrumented** already by existing reporting's daily count, so no requirement here delivers it.
...
An explicit negative result: **no service-level, latency, availability, conversion, retention or revenue figure is declared anywhere in the platform** — the search is below. Any target must be set by the Product Owner.

## 15. Suggested Sequencing

First, CA-1 Reorder from a past order with CA-4 Change awareness before commit and CA-7 Storefront integration surface: a repeat without the disclosure is the worse half, so they go together. Second, CA-2 Regularly purchased items, which sets the rule everything downstream waits for. Third, CA-6 Recurring demand visibility, which follows because one computation feeds both.

Two run alongside: CA-3 Saved reorder lists depends on nothing else and parallels any step after the first; CA-5 Reorder reminders needs the regular-items rule, so follows the second and parallels the third.
...
- **Four contradictions in existing material**, reported not corrected, none asserted here: guidance names a documentation framework the platform lacks; the documentation package's own introduction gives a different project home than the platform declares; that guidance calls any stored-shape change breaking while the vendor's published policy allows additive ones on a minor line; and money is whole numbers in the smallest unit of currency by default, not decimal. Evidence below.
...
- **Whether every citation resolved.** Every one did, for existence and for its claim.
...
- **How current this is.** Two later patch releases fix defects on these paths; upgrade first.
...
Measured: 1 sentence, 48 words, 302 characters, 4 clauses, 3 top-level and quotable as origins; the fourth sits inside the first.
...
Measured body word count: 2973, to the marker closing the body.
...
- **four separate data engines, a fifth engine** — the four registered test initialisers [e2e-common/test-config.ts:L32-L35] run as four jobs [.github/workflows/build_and_test.yml:L174-L311], and the file-backed engine absent from them [packages/core/package.json:L94].
...
- **Benchmark warm-up settings.** The single automated benchmark sets warm-up time and iteration counts for its own harness [packages/core/e2e/default-search-plugin.bench.ts:L60-L61, L86-L87]. These configure a measurement, not an expectation of one.
- **Descriptive scaling prose.** Three guide pages name throughput as a consequence of horizontal scaling, in each case without a figure [docs/docs/guides/deployment/horizontal-scaling.mdx:L12, L17; docs/docs/guides/developer-guide/worker-job-queue/index.mdx:L139; docs/docs/guides/core-concepts/job-queue/index.mdx:L54].
...
- **No prose or Markdown linter exists.** The staged-file lint configuration covers only TypeScript and HTML [.lintstagedrc.json:L2-L8], the formatter declares no Markdown override [.prettierrc:L1-L8], and all repository hooks are large-file-storage shims. There is therefore no linter to run against this file, and none was added.
...
2. **A repository address the manifests contradict.** The documentation package's readme links to `github.com/vendure-ecommerce/vendure` [docs/README.md:L30], while the root and core manifests both declare `github.com/vendurehq/vendure` [package.json:L9-L11; packages/core/package.json:L5-L7].
